...
/**
* @function customeSend: send a packet(transactionId, data size, data) to server get transactionID
*						to respone client and specify data size(create buff size) to receive data
*
...
/**
* @function customeRecv: receive a packet(transactionId, data size, data) from server
*						to get transactionID respone and specify data size(create buff size) to receive data
*
...
int customRecv(SOCKET &socket, string &data) {
	...
	do { // check transactionId respone and request is equal or not to specify packet return
		data = "";
		bytes = 0;
...
*
* @param rs: code return from server
*
**/
void customResult(int rs) {
	...
	case CODE_LOGOUT_FAIL_NOT_SINGED:
		...
		break;
	default:
		cout << "Error! The system has some error. Please try again and check format message!!!" << endl;
		break;
	}
}
